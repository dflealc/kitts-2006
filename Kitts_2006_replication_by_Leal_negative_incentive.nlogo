...
[
 ...
 n+_is      ;-->   the number of peers that actor i expects to shirk if she promotes work
 n-_i       ;-->   the number of peers that actor i expects to work because of her downward pressure to work
 n-_is      ;-->   the number of peers that actor i expects to shirk if she opposes work
...
to compute-peers-currently-working
 ...
 set s_i count other turtles with [working_i = 0] ;; count peers shirking
...
to compute-inclination-to-work
 ...
 [set IW_i ((g - c) + mu * (1 - lambda * (s_i / (s_i + 1))))]
...
ifelse negative-incentive?
[
 ifelse working_i = 1    ; if working and negative-incentive? is on:
 [set Ppromote_i ((s_i - n+_is) * g - ((n+_i / (n+_i + 1)) - (n_i / (n_i + 1))) * lambda * mu - enf)] ;; actor i pays attention to peers' contribution to work, enforce cost, and loss of incentive to fellow workers
 [set Ppromote_i ((s_i - n+_is) * g - enf)]  ;; (else) if shirking, only pays attention to peers' contribution to work, and enforce cost
]
[
 ifelse working_i = 1    ; if working and negative-incentive? is off:
...
to compute-payoff-for-opposing   ;; equation 7 in the article
 ...
 [
  ifelse working_i = 1   ; if working and negative-incentive? is on:
   [set Poppose_i ((s_i - n-_is) * g - ((n-_i / (n-_i + 1)) - (n_i / (n_i + 1))) * lambda * mu - enf)] ;; actor i pays attention to peers' contribution to work, enforce cost, and loss of incentive to fellow workers
   [set Poppose_i ((s_i - n-_is) * g - enf)]  ;; (else) if shirking, only pays attention to peers' contribution to work, and enforce cost
 ]
 [
  ifelse working_i = 1   ; if working and negative-incentive is off?
